...
export function createCartProduct(product, props) {
  ...
  cartProduct.innerHTML = `
    ...
    <img
      ...
      />
      <div class="content-layout d-flex-col">
        <div class="card-content d-flex-col">
          <p class="title">${product.title}</p>
          <p class="size">Size: <span>${props.size}</span></p>
          <div class="color d-flex-row" style="gap:4px"> <p>Color: </p><div style="background-color:${
            props.color
          }; height:18px; width:46px;"></div> 
          </div>
        </div>
        <div class="price-layout d-flex-row flex-wrap">

        ${
          product.discount != 0
            ? `
                <p class="price price-discount product_price">$${(
                  getDiscountedPrice(product.price, product.discount) *
                  props.quantity
                ).toFixed(2)}
                <p class="price price-main product_price product_price-old">$${
                  product.price
                }</p>
                ...
                `
            : `
            <p class="price price-main price-discount product_price">$${(
        ...
        <div class="item-price d-flex-row flex-wrap"><p>Per item:</p> <p>$${getDiscountedPrice(
          product.price,
          product.discount
        ).toFixed(2)}</p></div>
      ...
      <a class="product-actions d-flex-col justfy-sp-between ">
      <div class="deleteIcon">
        ...
        ${createQtyPill(props.quantity).outerHTML}
    </a>`;

  cartProduct.querySelector(".card-left").addEventListener("click", () => {
    let selectedProduct = {
      product: product,
      props: props,
    };
